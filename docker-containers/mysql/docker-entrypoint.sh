...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		mysql_check_config "$@"
		mysql_note "mysql_check_config"
		# Load various environment variables
		docker_setup_env "$@"
		mysql_note "docker_setup_env"
		docker_create_db_directories
		mysql_note "docker_create_db_directories"
		...
		mysql_note "DATABASE_ALREADY_EXISTS = $DATABASE_ALREADY_EXISTS"
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			docker_verify_minimum_env
			mysql_note "docker_verify_minimum_env"

			# check dir permissions to reduce likelihood of half-initialized database
			ls /docker-entrypoint-initdb.d/ > /dev/null
			mysql_note "docker-entrypoint-initdb.d"

			docker_init_database_dir "$@"
			mysql_note "docker_init_database_dir"
